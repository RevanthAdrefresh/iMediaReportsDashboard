...
const AdWidget = () => {
  ...
  if (!data.length) return <h3 style={{ color: "#777" }}>No AdWidget data available</h3>;

  const totalImpressions = data.reduce((a, b) => a + (+b.Impressions || 0), 0);
  const totalClicks = data.reduce((a, b) => a + (+b.Clicks || 0), 0);
  const totalSpend = data.reduce((a, b) => a + (+b.Spend || 0), 0);
  const pieData = [
    { name: "Impressions", value: totalImpressions },
    { name: "Clicks", value: totalClicks },
    { name: "Spend", value: totalSpend },
        ...
        <table style={styles.table}>
          ...
          <tbody>
            {data.map((row, i) => (
              <tr key={i}>
                {Object.keys(row).map((key, j) => (
                  <td key={j} style={styles.td}>{row[key]}</td>
                ))}
              </tr>
            ))}
          </tbody>
